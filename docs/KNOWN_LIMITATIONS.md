...
* **AudioBufferQueue Deadlock (Issue #30)**: Tests hang at `testDequeue_EmptyQueueAfterDequeuing` due to a deadlock in `_isEmpty` calling `isEmpty` which re-acquires the NSLock (non-reentrant).
  * *Context*: [2026-01-19] Identified during test suite analysis. Root cause: `_isEmpty` should check `count == 0` directly instead of calling `isEmpty`.
  * *Status*: OPEN. See GitHub Issue #30.
