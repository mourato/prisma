...
public final class AudioMerger {
    ...
    public func mergeAudioFiles(inputURLs: [URL], to outputURL: URL) async throws -> URL {
        logger.info("Merging \(inputURLs.count) audio files to: \(outputURL.path)")
        ...
        if existingURLs.count == 1 {
            return try await convertToM4A(inputURL: existingURLs[0], outputURL: outputURL)
        }
        ...
        for (index, url) in existingURLs.enumerated() {
            ...
            do {
                ...
                guard let track = tracks.first else {
                    logger.warning("No audio track found in: \(url.lastPathComponent)")
                    continue
                }
                ...
                guard let compositionTrack = composition.addMutableTrack(
                    ...
                    logger.warning("Failed to add track for: \(url.lastPathComponent)")
                    continue
                }
                ...
                logger.info("Added track: \(url.lastPathComponent) (\(duration.seconds)s)")
                
            } catch {
                logger.warning("Failed to load asset \(url.lastPathComponent): \(error.localizedDescription)")
            }
        }
    ...
    public func convertToM4A(inputURL: URL, outputURL: URL) async throws -> URL {
        logger.info("Converting \(inputURL.lastPathComponent) to M4A")
        
        let asset = AVAsset(url: inputURL)
        return try await exportAsset(asset, to: outputURL)
    }
    
    // MARK: - Private Methods
    
    private func exportComposition(_ composition: AVComposition, to outputURL: URL) async throws -> URL {
        return try await exportAsset(composition, to: outputURL)
    }
    ...
    private func exportAsset(_ asset: AVAsset, to outputURL: URL) async throws -> URL {
        ...
        exportSession.audioMix = try await createAudioMix(for: asset)
